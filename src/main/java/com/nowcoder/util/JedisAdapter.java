...
public class JedisAdapter implements InitializingBean {
    ...
    public static void main(String[] args) {
        ...
        jedis.setex("hello2", 15, "world"); //有过期时间
        print(1, jedis.keys("*"));
        try {
            Thread.sleep(1000);
        ...
        // 数值操作
        jedis.set("pv", "100");
        jedis.incr("pv"); //在内存中先增加，周期性返回sql。不要再sql中每次都update。
        jedis.decrBy("pv", 5);
        ...
        jedis.del(listName);
        for (int i = 0; i < 10; ++i) {
            jedis.lpush(listName, "a" + String.valueOf(i)); //从左边推元素进去
        }
        print(4, jedis.lrange(listName, 0, 12)); // 从左开始0-12元素
        print(5, jedis.llen(listName));
        print(6, jedis.lpop(listName));
        print(7, jedis.llen(listName));
        print(8, jedis.lrange(listName, 2, 6)); // 从左到右2-6
        print(9, jedis.lindex(listName, 3));
        print(10, jedis.linsert(listName, BinaryClient.LIST_POSITION.AFTER, "a4", "xx")); //在固定位置插入数据
        ...
        jedis.hsetnx(userKey, "school", "zju"); //hset if not exist，如果不存在就设置，存在不更新
        ...
        print(22, jedis.sunion(likeKey1, likeKey2)); //求并集 可以用来求共同好友，共同规范
        print(23, jedis.sdiff(likeKey1, likeKey2));//求互异 likeKey1有的，likeKey2没有的
        print(23, jedis.sdiff(likeKey2, likeKey1));//求互异 likeKey2有的，likeKey1没有的
        ...
        print(38, jedis.zrank(rankKey, "Ben")); //从小到大，排第几名，从0开始
        print(39, jedis.zrevrank(rankKey, "Ben")); //从大到小
        ...
        */
        ...
        for (int i = 0; i < 100; ++i) {
            Jedis j = pool.getResource(); //默认8条线程
            j.get("a");
    ...
    public long sadd(String key, String value) { //将点赞用户的集合放进Redis的集合中
        Jedis jedis = null;
        try {
            jedis = pool.getResource(); //从池里取一个
